...
import Listas.ArrayUnorderedList;
import Queues.PriorityQueue;
import estruturasADT.NetworkADT;
import estruturasADT.UnorderedListADT;
...
public class Network<T> extends MatrixGraph<T> implements NetworkADT<T> {
    ...
    public void setEdgeWeight(T firstVertex, T secondVertex, double weight) {
        ...
        if (secondVertex.equals("exterior") || firstVertex.equals("exterior") || secondVertex.equals("entrada")
                || firstVertex.equals("entrada")) {
    ...
    public Iterator<T> iteratorShortestWeight(T startVertex, T targetVertex)
            throws UnknownPathException, EmptyCollectionException {
        return shortestPathWeight(startVertex, targetVertex).iterator();
    }

    @Override
    public ArrayUnorderedList<T> shortestPathWeight(T vertex1, T vertex2)
            throws EmptyCollectionException, UnknownPathException {
    ...
    public Network mstNetwork() {
        ...
        while ((resultGraph.size() < this.size()) && !minHeap.isEmpty()) {
            // Get the edge with the smallest weight that has exactly one vertex already in
            // the resultGraph
            do {
                try {
